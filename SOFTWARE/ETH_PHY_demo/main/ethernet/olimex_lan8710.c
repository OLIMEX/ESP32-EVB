...
/* Value of MII_PHY_IDENTIFIER_REGs for Microchip LAN8710
 * (Except for bottom 4 bits of ID2, used for model revision)
 */
#define LAN8710_PHY_ID1 0x0007
#define LAN8710_PHY_ID2 0xc0f0
#define LAN8710_PHY_ID2_MASK 0xFFF0

/* LAN8710-specific registers */
...
static const char *TAG = "lan8710";

void phy_lan8710_check_phy_init(void)
{
...
eth_speed_mode_t phy_lan8710_get_speed_mode(void)
{
    if(esp_eth_smi_read(PHY_SPECIAL_CONTROL_STATUS_REG) & SPEED_INDICATION_100T) {
        ESP_LOGD(TAG, "phy_lan8710_get_speed_mode(100)");
        return ETH_SPEED_MODE_100M;
    } else {
        ESP_LOGD(TAG, "phy_lan8710_get_speed_mode(10)");
        return ETH_SPEED_MODE_10M;
    }
}

eth_duplex_mode_t phy_lan8710_get_duplex_mode(void)
{
    if(esp_eth_smi_read(PHY_SPECIAL_CONTROL_STATUS_REG) & DUPLEX_INDICATION_FULL) {
        ESP_LOGD(TAG, "phy_lan8710_get_duplex_mode(FULL)");
        return ETH_MODE_FULLDUPLEX;
    } else {
        ESP_LOGD(TAG, "phy_lan8710_get_duplex_mode(HALF)");
        return ETH_MODE_HALFDUPLEX;
    }
}

void phy_lan8710_power_enable(bool enable)
{
#if 0
    if (enable) {
...
void phy_lan8710_dump_registers()
{
    ESP_LOGD(TAG, "LAN8710 Registers:");
